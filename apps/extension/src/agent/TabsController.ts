...
export class TabsController {
	...
	async init(task: string, options: TabsInitOptions = {}) {
		...
		if (experimentalIncludeAllTabs) {
			...
			if (!allTabs?.success) {
				throw new Error(`Failed to get window tabs: ${allTabs?.error ?? 'background script unreachable'}`)
			}
	...
	async openNewTab(url: string, options: { signal?: AbortSignal } = {}): Promise<string> {
		...
		if (!result?.success) {
			throw new Error(`Failed to open new tab: ${result?.error ?? 'background script unreachable'}`)
		}
	...
	async switchToTab(tabId: number): Promise<string> {
		...
		// Make Chrome actually focus the tab in the browser window.
		await sendMessage({
			type: 'TAB_CONTROL',
			action: 'activate_tab',
			payload: { tabId },
...
// signal is observed once per poll tick, not during an in-flight check().
